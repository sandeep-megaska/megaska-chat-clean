// /api/chat.js — MEGHA Smart Fast Path (Edge, brand-grounded, no sources)
...
// ---------- Brand Brain (edit any line to your exact copy) ----------
const BRAND = {
  name: "MEGASKA",
  whatWeDo:
...
// ---------- lightweight keyword search (fast) ----------
...
// ---------- embeddings (used only if needed) ----------
...
// ---------- answer builders (fast, brand-first) ----------
function answerFromBrand(i, msg) {
  switch (i) {
    case 'delivery':
...
// ---------- OpenAI final polish (short + on-brand) ----------
...
export default async function handler(req) {
  ...
  try {
    ...
    // 1) Fast intent answer from Brand Brain (instant)
    const i = intent(message);
    let base = answerFromBrand(i, message);

    // 2) Quick brand-specific enrichment (keyword first, then embeddings if needed)
    let extra = '';
    try {
      // keyword hits are super fast and often enough
      const [pHits, cHits] = await Promise.all([
        kwPages(i === 'delivery' ? 'shipping' : i, 4),
        kwChunks(i === 'delivery' ? 'shipping' : i, 6)
      ]);
      const snippets = (cHits || []).slice(0, 3).map(x => (x.content || '').slice(0, 300));
      if (snippets.length) extra = `\n\n${snippets.join('\n\n')}`;
    } catch {}

    // if still thin and the question is long/specific, try embeddings quickly
    ...
    // 3) Final polish (short, owner voice)
    const reply = await polish(message, `${base}${extra}`);
